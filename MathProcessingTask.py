...
class MathProcessing(TaskTemplate):
    yes_key_name = "espace"
    yes_key_code = "space"
    quit_code = "q"
    ...
    def task(self, no_trial, exp_start_timestamp, trial_start_timestamp, practice=False):
        ...
        numbers = [str(randint(1, 6)) for i in range(4)]
        print(numbers)
        op = ["+", "-", "*"]
        shuffle(op)
        op_final = list(itertools.chain.from_iterable(itertools.zip_longest(numbers, op)))
        op_final.pop(-1)
        op_final = ''.join(op_final)
        result = eval(op_final)
        ...
        self.win.flip()
        core.wait(1)
        self.create_visual_text(op_final).draw()
        self.win.flip()
